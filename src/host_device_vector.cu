...
public:
    ...
    void Fill(T v) {  // NOLINT
        if (HostCanWrite()) {
            ...
            gpu_access_ = GPUAccess::kWrite;
            SetDevice();
            auto s_data = common::ToSpan(*data_d_);
            common::LaunchN(data_d_->size(),
    ...
    void Extend(HostDeviceVectorImpl* other) {
        ...
        if (HostCanWrite() && other->HostCanRead()) {
            ...
            cuda_handler(cudaMemcpyAsync(this->DevicePointer() + ori_size,
                ptr,
                other->Size() * sizeof(T),
    ...
    void LazySyncHost(GPUAccess access) {
        ...
        SetDevice();
        cuda_handler(cudaMemcpy(data_h_.data(),
            data_d_->data().get(),
    ...
    void LazySyncDevice(GPUAccess access) {
        ...
        LazyResizeDevice(data_h_.size());
        SetDevice();
        cuda_handler(cudaMemcpyAsync(data_d_->data().get(),
            data_h_.data(),
            data_d_->size() * sizeof(T),
...
private:
    ...
    void CopyToDevice(HostDeviceVectorImpl* other) {
        if (other->HostCanWrite()) {
            ...
            gpu_access_ = GPUAccess::kWrite;
            SetDevice();
            cuda_handler(cudaMemcpyAsync(data_d_->data().get(), other->data_d_->data().get(),
    ...
    void CopyToDevice(const T* begin) {
        ...
        gpu_access_ = GPUAccess::kWrite;
        SetDevice();
        cuda_handler(cudaMemcpyAsync(data_d_->data().get(), begin,
            data_d_->size() * sizeof(T), cudaMemcpyDefault));
    }
    ...
    void SetDevice() {
        ...
        if (cudaSetDeviceHandler == nullptr) {
            ...
            //cuda_handler(cudaSetDevice(device_));
        ...
        if (!data_d_) {
            data_d_.reset(new thrust::device_vector<T>);
        }
    }
};
...
template class HostDeviceVector<LBitField64>;
